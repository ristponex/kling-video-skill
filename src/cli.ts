...
/**
 * Kling Video CLI — Generate videos using Kling AI via Atlas Cloud API
 *
 * Usage:
 *   kling-video "prompt" [options]
 *
 * Options:
 *   --model        Model variant (default: kwaivgi/kling-v3.0-pro/text-to-video)
 *   --mode         Generation mode: t2v, i2v, ref, edit (default: t2v)
 *   --duration     Video duration in seconds: 5, 10 (default: 5)
 *   --resolution   Output resolution: 480p, 720p, 1080p, 4k (default: 1080p)
 *   --aspect-ratio Aspect ratio: 16:9, 9:16, 1:1, 4:3, 3:4, 21:9 (default: 16:9)
 *   --audio        Enable native audio generation (default: false)
 *   --image        Input image/video path for i2v/ref/edit modes
 *   --output       Output file path (default: ./output/{timestamp}.mp4)
 *   --help         Show help message
 */

const API_BASE = "https://api.atlascloud.ai/api/v1/model/prediction";
const DEFAULT_MODEL = "kwaivgi/kling-v3.0-pro/text-to-video";
...
const MODEL_MAP: Record<string, Record<string, string>> = {
  ...
  "kwaivgi/kling-v3.0-std": {
    t2v: "kwaivgi/kling-v3.0-std/text-to-video",
  ...
  "kwaivgi/kling-v2.5-turbo-pro": {
    t2v: "kwaivgi/kling-v2.5-turbo-pro/text-to-video",
  },
  "kwaivgi/kling-o3-pro": {
    t2v: "kwaivgi/kling-o3-pro/text-to-video",
    i2v: "kwaivgi/kling-o3-pro/image-to-video",
  },
  "kwaivgi/kling-o1": {
    t2v: "kwaivgi/kling-o1/text-to-video",
  },
};
...
function parseArgs(args: string[]): CLIArgs {
  ...
  if (!prompt) {
    ...
    console.error('Usage: kling-video "your prompt" [options]');
    process.exit(1);
  }
  ...
  return {
    ...
    output: getFlag("output", `./output/kling_${timestamp}.mp4`),
  };
}
...
Options:
  --model <id>          Model variant (default: kwaivgi/kling-v3.0-pro/text-to-video)
  --mode <mode>         Generation mode: t2v, i2v, ref, edit (default: t2v)
  --duration <seconds>  Video duration: 5, 10 (default: 5)
  --resolution <res>    Output resolution: 480p, 720p, 1080p, 4k (default: 1080p)
  --aspect-ratio <r>    Aspect ratio: 16:9, 9:16, 1:1, 4:3, 3:4, 21:9 (default: 16:9)
  --audio               Enable native audio generation
  --image <path>        Input image/video path for i2v/ref/edit modes
  --output <path>       Output file path (default: ./output/{timestamp}.mp4)
  --help                Show this help message

Examples:
  kling-video "A sunset over the ocean with gentle waves"
  kling-video "Clouds moving" --mode i2v --image ./photo.jpg
...
async function buildRequestBody(args: CLIArgs): Promise<Record<string, unknown>> {
  ...
  if ((args.mode === "i2v" || args.mode === "ref" || args.mode === "edit") && args.image) {
...
async function pollForResult(apiKey: string, requestId: string): Promise<PredictionResponse> {
  ...
  console.error("Video generation can take up to 5 minutes for complex prompts.");
  process.exit(1);
}
...
async function main(): Promise<void> {
  ...
  if ((args.mode === "i2v" || args.mode === "ref" || args.mode === "edit") && !args.image) {
    console.error(`Error: --image is required for ${args.mode} mode.`);
    process.exit(1);
  }

  // Auto-select model variant based on mode if using default
  if (args.model === DEFAULT_MODEL && args.mode === "i2v") {
    args.model = "kwaivgi/kling-v3.0-pro/image-to-video";
  ...
  console.log("");
  console.log("=== Kling Video Generator ===");
  console.log("");
